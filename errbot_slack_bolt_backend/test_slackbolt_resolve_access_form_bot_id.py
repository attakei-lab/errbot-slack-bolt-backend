...
class Test_resolve_access_form_bot_id:
    ...
    def test_not_resolve_access_form_bot_id(self, mocked_backend):
        mocked_backend.username_to_bot_id = MagicMock(return_value=self.wrong_bot_id)
        mocked_backend.resolve_access_form_bot_id()
        assert mocked_backend.bot_config.ACCESS_FORM_BOT_INFO.get('bot_id') == self.wrong_bot_id 
